...
Google Colabで Market Amedas → El Shaddai production audit → Parallax Engine を正式な3セル構成で再現実行する場合は、[`notebooks/production_control_notebook_v1.md`](notebooks/production_control_notebook_v1.md) を使用してください。FRED接続テスト、Market Amedas snapshot schema検証、成果物一覧、Parallax Summary / asset contexts / warnings、quality gate、manifest safety flags、Markdown表示、任意のGoogle Drive保存までを固定手順にしています。詳細なquality gateの読み方と成果物用途は [`PRODUCTION_RUNBOOK.md`](PRODUCTION_RUNBOOK.md) を参照してください。

## L.U.M.U.S.-8 Production Archive v1.0

Production Archive は、毎回の本番監査成果物を日付/実行時刻別に保存し、quality gate、データ状態、Parallax状態、warning、安全境界を履歴比較できるようにする監査証跡レイヤーです。保存・記録・索引化のみを行い、Market Amedas、El Shaddai、Parallax Engineの判断ロジックやスコアを変更せず、自動売買・自動売却・配分変更にも接続しません。

# El Shaddai 健全度診断システム v1.5.1

El Shaddai は、三体戦略における監査対象資産を **Permanent Holdings / Value Opportunity Only** と **Role-Audited Assets** に分け、説明可能性を優先して診断する v1.5.1 初期版です。売買予測やブラックボックスの機械学習モデルではなく、スポット買い好機、資産の役割健全性、元気玉（追加投資資金）の投入先候補を人間が監査できる形で提示します。

v1.5.1 は「実運用完成版」ではなく、透明性・説明可能性・監査可能性・将来拡張性を優先した器です。
...
## Role Score aggregation（v1.5.1）

El Shaddai v1.5.1 の Role Score は、単純平均ではなく、資産ごとの Role structure に基づく監査スコアです。各 component は `core` / `support` / `context` / `risk_penalty` に分類され、Markdown レポートの `Role Component Weights` に weight と group が一覧出力されます。
...
## TIP I.N.F.E.R.N.O. adapter（v1.5.1）

El Shaddai v1.5.1 では、TIP Role Score に I.N.F.E.R.N.O.（Inflation Navigation Framework for Evaluating Real-rate Neutralization Operations）adapter を接続できます。TIP の Role は「魔法防御（Magic Resistance）」、すなわち購買力防衛です。インフレ、通貨価値の希薄化、スタグフレーションから資産を守る Summer（インフレ気団）の防衛資産として監査します。
...
## GLDM A.U.R.A. adapter（v1.5.1）

El Shaddai v1.5.1 では、GLDM Role Score に Project A.U.R.A. adapter を接続できます。A.U.R.A. adapter は、GLD と SPY/TLT/DBC/UUP/BTC-USD のローリング相関を使い、金が現在どのレジームで動いているかを推定します。目的は金価格の予測ではなく、GLDM が「無国籍価値保存」「通貨不信ヘッジ」「実物資産防衛」として機能しているかを説明可能に監査することです。
...
A.U.R.A. adapter は v1.5.1 で実データから生成できる proxy だけを上書きし、中央銀行買付や地政学リスクなど未接続の proxy は中立のまま保持します。


## External data dependency handling（v1.5.1）

El Shaddai v1.5.1 は、外部データについて以下の 3 モードを明確に分けます。
...
デフォルトの v1.5.1 はネットワーク依存を避け、テスト可能性と透明性を優先します。ただし `--use-lode-tlt-role` を指定した場合は FRED から L.O.D.E. 系列を取得して TLT Role proxy を生成し、`--use-inferno-tip-role` を指定した場合は FRED から I.N.F.E.R.N.O. 系列を取得して TIP Role proxy を生成し、`--use-aura-gldm-role` を指定した場合は yfinance から A.U.R.A. 価格データを取得して GLDM Role proxy を生成します。
...
- PNG 画像の直接出力（v1.5.1 は依存関係を増やさないため HTML/SVG 出力）
...
- BTC 専用のオンチェーン/サイクル指標
  ...
  - Reserve Risk
  - 前サイクル高値との距離 など
- Role proxy の実データ変換ロジック（v1.5.1 では TLT/L.O.D.E.、TIP/I.N.F.E.R.N.O.、GLDM/A.U.R.A. が初期接続済み）
- 日次履歴保存
- スコア推移グラフ

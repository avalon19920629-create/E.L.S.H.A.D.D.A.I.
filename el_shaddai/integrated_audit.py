...
PRICE_FLAGS = {"price_weakness", "drawdown", "price_decline"}
...
def confidence_multiplier(level: int | None) -> float:
    return {5: 1.05, 4: 1.02, 3: 1.0, 2: 0.92, 1: 0.85}.get(level or 3, 1.0)
...
def run_integrated_audit(asset_audits: Iterable[AssetAuditInput], portfolio: PortfolioInput, market_amedas: MarketAmedasInput | None = None) -> dict[str, Any]:
    ...
    for audit in audits:
        ...
        score = round(clamp(audit.role_health_score * regime * confidence_multiplier(audit.confidence_level) * penalty), 2)
        level = health_level_for(score)
        wound = classify_wound(score, audit.risk_flags, audit.wound_level)
        rows.append({
            "asset": audit.asset, "audit_engine": audit.audit_engine, "asset_health_score": score, "health_level": level,
            "health_label": HEALTH_LABELS_JA[level], "wound_level": wound, "wound_label": WOUND_TYPE_LABELS_JA[wound],
            "confidence_level": audit.confidence_level or 3, "confidence_label": CONFIDENCE_LABELS_JA[audit.confidence_level or 3],
    ...
    if len(severe) >= 2: global_level = min(global_level, 2)
    if collapsed or correlation_score < 60: global_level = min(global_level, 1)
    if defense_wounded >= 2 or inflation_wounded >= 2: global_level = min(global_level, 2)
    by_asset = {row["asset"]: row for row in rows}
    ...
    if growth_strong and defense_broadly_weak: global_level = min(global_level, 2)

    raw_action = {5: 0, 4: 0, 3: 1, 2: 2, 1: 3, 0: 4}[global_level]
    ...
    recommended = ["固定比率と既存の乖離ルールを維持する。", "負傷アセットと役割グループを次回監査で重点確認する。"]
    ...
    if context.market_context_flags: checkpoints.append("Market Amedasの文脈フラグと役割健全性を分離して再確認する。")
    if not correlation_available: checkpoints.append("相関データを取得し、相関構造の健全性を確認する。")

    result: dict[str, Any] = {
        "sanctuary_health_score": sanctuary, "global_judgment_level": global_level, "global_judgment_label": GLOBAL_JUDGMENT_LABELS_JA[global_level],
        ...
        "hysteresis_note": hysteresis_note, "asset_health_rank": rows, "wounded_assets": wounded, "role_group_diagnosis": _role_diagnosis(rows),
        ...
        "components": {"weighted_asset_health": round(weighted_health, 2), "role_coverage_factor": round(role_coverage_factor, 3), "correlation_integrity_factor": round(correlation_factor, 3), "concentration_penalty": round(concentration_penalty, 3)},
    }

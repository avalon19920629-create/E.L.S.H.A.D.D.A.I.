"""Command-line interface for El Shaddai v1.9."""
...
from .atlas_adapter import bndx_role_inputs_from_csv, latest_bndx_role_inputs
from .aura_adapter import gldm_role_inputs_from_csv, latest_gldm_role_inputs
from .gaia_adapter import dbc_role_inputs_from_csv, latest_dbc_role_inputs
...
from .text_sanitizer import safe_print


def diagnose_data_sources() -> int:
    """Print adapter dependency and provider diagnostics without failing the CLI."""

    safe_print("Data source diagnostics")
    try:
        __import__("yfinance")
        safe_print("yfinance import: ok")
    except Exception as exc:  # noqa: BLE001
        safe_print(f"yfinance import: unavailable ({exc})")
    try:
        import urllib.request

        with urllib.request.urlopen("https://fred.stlouisfed.org/graph/fredgraph.csv?id=T10Y2Y", timeout=10) as response:
            safe_print(f"FRED endpoint: ok (HTTP {response.status})")
    except Exception as exc:  # noqa: BLE001
        safe_print(f"FRED endpoint: unavailable ({exc})")
    atlas = latest_bndx_role_inputs()
    safe_print(f"A.T.L.A.S. fetch: {'ok' if atlas.used_atlas else 'fallback'}")
    for warning in atlas.warnings:
        safe_print(warning)
    lode = latest_tlt_role_inputs()
    safe_print(f"L.O.D.E. fetch: {'ok' if lode.used_lode else 'fallback'}")
    for warning in lode.warnings:
        safe_print(warning)
    inferno = latest_tip_role_inputs()
    safe_print(f"I.N.F.E.R.N.O. fetch: {'ok' if inferno.used_inferno else 'fallback'}")
    for warning in inferno.warnings:
        safe_print(warning)
    aura = latest_gldm_role_inputs()
    safe_print(f"A.U.R.A. fetch: {'ok' if aura.used_aura else 'fallback'}")
    for warning in aura.warnings:
        safe_print(warning)
    arcadia = latest_xlre_role_inputs()
    safe_print(f"A.R.C.A.D.I.A. fetch: {'ok' if arcadia.used_arcadia else 'fallback'}")
    for warning in arcadia.warnings:
        safe_print(warning)
    gaia = latest_dbc_role_inputs()
    safe_print(f"G.A.I.A. fetch: {'ok' if gaia.used_gaia else 'fallback'}")
    for warning in gaia.warnings:
        safe_print(warning)
    oracle = latest_oracle_inputs()
    safe_print(f"O.R.A.C.L.E. fetch: {'ok' if oracle.used_oracle else 'fallback'}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--use-atlas-bndx-role", action="store_true", help="Fetch A.T.L.A.S. market data and overwrite BNDX role proxy inputs.")
    parser.add_argument("--atlas-inputs-csv", help="Manual A.T.L.A.S. CSV for BNDX role proxies; takes priority over live yfinance.")
    ...
    parser.add_argument("--use-gaia-dbc-role", action="store_true", help="Fetch G.A.I.A. market data and overwrite DBC commodity-regime role proxy inputs.")
    parser.add_argument("--gaia-prices-csv", help="Manual G.A.I.A. prices CSV for DBC role proxies; takes priority over live yfinance.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    oracle_result = None
    gaia_result = None
    if args.atlas_inputs_csv or args.use_atlas_bndx_role:
        atlas_result = bndx_role_inputs_from_csv(args.atlas_inputs_csv) if args.atlas_inputs_csv else latest_bndx_role_inputs()
        role_inputs = {asset: dict(values) for asset, values in role_inputs.items()}
        role_inputs["BNDX"] = dict(atlas_result.role_inputs["BNDX"])
        source_summary += "; BNDX Role inputs generated by A.T.L.A.S. adapter"
        for warning in atlas_result.warnings:
            safe_print(warning)
    ...
    if args.gaia_prices_csv or args.use_gaia_dbc_role:
        gaia_result = dbc_role_inputs_from_csv(args.gaia_prices_csv) if args.gaia_prices_csv else latest_dbc_role_inputs()
        role_inputs = {asset: dict(values) for asset, values in role_inputs.items()}
        role_inputs["DBC"] = dict(gaia_result.role_inputs["DBC"])
        source_summary += "; DBC Role inputs generated by G.A.I.A. adapter"
        for warning in gaia_result.warnings:
            safe_print(warning)
    ...
    md_path = write_markdown(scores, output_dir, source_summary, atlas_result=atlas_result, lode_result=lode_result, inferno_result=inferno_result, aura_result=aura_result, gaia_result=gaia_result, arcadia_result=arcadia_result, oracle_result=oracle_result)
    html_path = write_html(scores, output_dir, aura_result=aura_result, gaia_result=gaia_result, arcadia_result=arcadia_result, oracle_result=oracle_result)
    safe_print(f"Wrote {csv_path}")
    safe_print(f"Wrote {md_path}")
    safe_print(f"Wrote {html_path}")
    return 0
